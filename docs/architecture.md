...
3. **The training pipeline** (upcoming) — LibTorch-based local inference, LoRA
   fine-tuning, and adapter lifecycle management, driven from inside the graph.
...
| `trace` | structured trace capture from executor runs, the SQLite trace store, the trace→example dataset builder, and a graph channel that accumulates training examples during execution |
...
Built-in verifier kinds — exact match, numeric tolerance, result-set comparison,
subprocess exit code, LLM-judge — cover most needs; a custom verifier is a function.

The eval harness fans an agent across a suite concurrently, scores each instance, and
aggregates into **accuracy ± a 95% confidence interval**, pinning everything needed to
reproduce the number: model, seed, framework version, and git commit. Reports persist
in SQLite, a run can be recorded as the suite's **baseline**, and two runs can be
compared for a signed delta. Verified traces export directly as JSONL training data —
the input to the upcoming `finetune` feature.
...
- **Testing**: everything is testable offline — providers through the mock backend,
